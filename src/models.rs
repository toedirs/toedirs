...
impl UserPreferences{ 
        ...
        pub fn calculate_load(&self, heartrates: Vec<u32>)->u32{
            ...
            let hr_buckets = heartrates.iter().filter(|&hr| *hr as f64 > self.max_heartrate as f64 * 0.55).fold(HashMap::new(),|mut buckets: HashMap<_,usize>, hr|{
                let count = buckets.entry(hr).or_insert(0);
                *count += 1;
                buckets
            });
            hr_buckets.iter().map(|(&hr,time_s)|(self.c * (self.tau * *hr as f64).exp() +1.0) * *time_s as f64 / 60.0).sum::<f64>().round() as u32
        }
}

impl Default for UserPreferences{
    fn default() -> Self {
        Self { user_id: 0, start_time: None, end_time: None, aerobic_threshold: 155, anaerobic_threshold: 172, max_heartrate: 183, tau: 0.0809749, c:  0.000002370473}
    }
}


#[cfg(feature = "ssr")]
pub async fn get_user_preferences(user_id: i64, date: DateTime<Local>,
    executor: impl sqlx::PgExecutor<'_>,
)->UserPreferences{
    let result = sqlx::query_as!(UserPreferences, 
        r#"
        SELECT
            user_id,
        ...
        LIMIT 1
        "#,
        user_id as i32,date).fetch_optional(executor).await.expect("couldn't query user prefences").unwrap_or_default();
    return result;    
}
